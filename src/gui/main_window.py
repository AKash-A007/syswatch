...
from PySide6.QtWidgets import (
    ...
    QSystemTrayIcon, QStyle, QApplication, QFileDialog
)
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        main_splitter.setSizes([300, 800, 450])
    ...
    def export_data(self):
        path, _ = QFileDialog.getSaveFileName(self, "Export Services Data", "services.csv", "CSV Files (*.csv)")
        if not path:
            return
        try:
            with open(path, "w", newline="", encoding="utf-8") as f:
                ...
                for s in self.service_panel.current_services.values():
                    writer.writerow([
                        ...
                        s.get("cpu_usage"), s.get("memory_usage"), s.get("uptime_seconds")
                    ])
    ...
    def run_stress_test(self):
        reply = QMessageBox.question(self, "Stress Test", "This will generate high load on API Gateway. Continue?", QMessageBox.Yes | QMessageBox.No)
    ...
    def open_settings(self):
        ...
        if dialog.exec():
            ...
            self.metrics_view.update_timer.start(int(self.config.get("metrics_refresh_interval", 1) * 1000))
            self.update_status_bar_connection()
    ...
    def closeEvent(self, event):
        if self.config.get("confirm_exit"):
            reply = QMessageBox.question(self, "Confirm Exit", "Are you sure you want to exit SysWatch Pro?", QMessageBox.Yes | QMessageBox.No)
